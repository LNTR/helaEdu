...
export default function TableRowC({ complaintId, comment, complaint, commentedBy, reportedBy, date, status,articleId }) {
  ...
  return (
    <div className='bg-white border border-blue text-black rounded-3xl mx-32 my-4 px-7 py-4'>
      ...
      {showDeletePopup && (
        <DeletePopup onCancel={() => setShowDeletePopup(false)} onDelete={handleDelete} />
      )}
      {showFeedbackPopup && (
        <FeedbackPopup
          onCancel={() => setShowFeedbackPopup(false)}
          onSubmitFeedback={handleAcceptDecline}
        />
      )}
    </div>
  );
